...
def solve_daily_routing(data, historical_km, lambda_penalty, mu_penalty):
    ...
    min_capacity = min(data["vehicle_capacities"])
    for v in range(data["num_vehicles"]):
        fixed_cost = int(lambda_penalty * historical_km[v] + mu_penalty * (data["vehicle_capacities"][v] - min_capacity))
        routing.SetFixedCostOfVehicle(fixed_cost, v)
    ...
    daily_distances = []
    max_index = manager.GetNumberOfIndices()
    for v in range(data["num_vehicles"]):
        index = routing.Start(v)
        route_distance = 0
        while not routing.IsEnd(index):
...
def print_daily_solution(data, manager, routing, solution):
    ...
    for v in range(data["num_vehicles"]):
        index = routing.Start(v)
        route_distance = 0
        route_log = f"Route for vehicle {v}:\n"
        while not routing.IsEnd(index):
            node = manager.IndexToNode(index)
            arrival = solution.Value(time_dimension.CumulVar(index))
            current_cap = solution.Value(capacity_dimension.CumulVar(index))
            next_index = solution.Value(routing.NextVar(index))
            delivered = 0 if node == data["depot"] else (current_cap - solution.Value(capacity_dimension.CumulVar(next_index)))
            route_log += f" Node {node} (Arrival Time: {arrival}, Capacity: {current_cap}, Delivered: {delivered}) ->"
            prev = index
            index = next_index
        ...
        arrival = solution.Value(time_dimension.CumulVar(index))
        final_cap = solution.Value(capacity_dimension.CumulVar(index))
        route_log += f" Node {node} (Arrival Time: {arrival}, Capacity: {final_cap}, Delivered: 0)\n"
        route_log += f"Distance of the route: {route_distance}\n"
        if route_distance > 0:
